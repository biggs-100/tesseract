...
impl PageCache {
    ...
    pub fn get(&self, key: &PageKey) -> Option<Page> {
    ...
    pub fn remove(&self, key: &PageKey) -> Option<Page> {
        let mut cache = self.inner.lock().expect("page cache lock poisoned");
        cache.pop(key)
    }
...
mod tests {
    ...
    fn eviction_removes_lru_entry() {
        ...
        cache.insert(k_a.clone(), p_a);
        cache.insert(k_b.clone(), p_b);
        cache.insert(k_c.clone(), p_c);

        // Access A to promote it — makes B the LRU
        cache.get(&k_a);

        // Insert D — should evict B (the LRU)
        cache.insert(k_d.clone(), p_d);

        assert!(cache.get(&k_a).is_some(), "A should survive (was promoted)");
        assert!(cache.get(&k_b).is_none(), "B should be evicted (LRU)");
        assert!(cache.get(&k_c).is_some(), "C should survive");
        assert!(cache.get(&k_d).is_some(), "D should be present");
    }
    ...
    fn get_promotes_entry() {
        ...
        assert!(cache.get(&k_a).is_some(), "A should survive");
        assert!(cache.get(&k_b).is_some(), "B should survive (was promoted)");
        assert!(cache.get(&k_c).is_none(), "C should be evicted (LRU)");
        assert!(cache.get(&k_d).is_some(), "D should be present");
    }
    ...
    fn clear_empties_cache() {
        ...
        cache.insert(k_a, p_a);
        cache.insert(k_b, p_b);
