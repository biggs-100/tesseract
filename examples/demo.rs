...
async fn main() {
    ...
    let categories = ["science", "history"];
    ...
    println!();
    println!("     Results with topological bias: {} found", result_biased.results.len());
    for r in &result_biased.results {
        println!("       id={:>2}  score={:.4}  ← history vector", r.id, r.score);
    }

    print_step(4, "Verify: biased search rescued results from post-filter death...");
    let science_count = result_biased.results.iter().filter(|r| r.id <= 5).count();
    let history_count = result_biased.results.iter().filter(|r| r.id > 5).count();
    println!(
        "     science={}, history={} → bias shifted query toward history region ✓",
        science_count, history_count
    );
